...
var LoginController = /** @class */ (function () {
    ...
    LoginController.prototype.isLogin = function (req, res) {
        ...
        res.json(helper_1.getResponseData('', isLogin));
    };
    LoginController.prototype.login = function (req, res) {
        var password = req.body.password;
        var isLogin = LoginController_1.isLogin(req);
        if (isLogin) {
            res.json(helper_1.getResponseData('已经登录过', true));
        }
        else {
            if (password === '123' && req.session) {
                req.session.login = true;
                res.json(helper_1.getResponseData('登录成功', true));
            }
            else {
                res.json(helper_1.getResponseData('密码错误', false));
            }
        }
    };
    LoginController.prototype.logout = function (req, res) {
        ...
        res.json(helper_1.getResponseData('退出登录', true));
    };
